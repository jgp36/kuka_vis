...
int main(int argc, char ** argv) {
  ...
  char* def_in_address = "/visualize_proxy_green";
  if (argc > 2) {
    def_in_address = argv[1];
  }

  char* def_out_address = "proxy_marker_green";
  if (argc > 3) {
    def_out_address = argv[2];
  }

  ros::init(argc, argv, "proxy_display_green");
  ros::NodeHandle node("~");
  ...
  for (size_t ii(0); ii < 8; ++ii) {
    ...
    robot_markers.markers[ii].color.g = 0.9;
